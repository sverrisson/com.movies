...
export default class MovieCell extends Lightning.Component {
  static _template() {
    return {
      Shadow: {
        alpha: 0,
  ...
  _init() {
    const name = this.movie.original_title
    this.tag('Label').patch({ text: name })
    const poster_path = Model.imageUrl(this.movie.poster_path)
    this.tag('Image').patch({ src: poster_path })
  ...
  _handleEnter() {
    this.fireAncestors('$movieSelected', this.movie.id)
  }
}
